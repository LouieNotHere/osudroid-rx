...
public void onUpdate(final float pSecondsElapsed) {
    float interpolationFactor = Math.min(1.0f, pSecondsElapsed / 0.016f); // Assuming ~60 FPS (0.016s per frame)

    for (int i = 0; i < CursorCount; i++) {
        Cursor cursor = cursors[i];

        // Smooth interpolation for movement
        cursor.mousePos.x = cursor.mousePos.x * interpolationFactor + cursor.mousePos.x * (1 - interpolationFactor);
        cursor.mousePos.y = cursor.mousePos.y * interpolationFactor + cursor.mousePos.y * (1 - interpolationFactor);

        // Pass cursor state to sprite for visual feedback
        if (cursorSprites != null) {
            cursorSprites[i].setPosition(cursor.mousePos.x, cursor.mousePos.y);
            cursorSprites[i].setShowing(cursor.mouseDown);
        }
    }
}

private float findClosestObjectTime(PointF cursorPos) {
    float closestTime = Float.MAX_VALUE;
    for (GameObject object : activeObjects) {
        float timeDiff = Math.abs(object.getTime() - GameHelper.getGlobalTime());
        if (timeDiff < closestTime) {
            closestTime = object.getTime();
        }
    }
    return closestTime;
}
...
public boolean onSceneTouchEvent(final Scene pScene, final TouchEvent pTouchEvent) {
    int pointerId = pTouchEvent.getPointerID();
    if (pointerId >= 0 && pointerId < CursorCount) {
        Cursor cursor = cursors[pointerId];

        switch (pTouchEvent.getAction()) {
            case TouchEvent.ACTION_DOWN: // Touch started
                cursor.mouseDown = true;
                cursor.mousePos.set(pTouchEvent.getX(), pTouchEvent.getY());

                // Calculate offset between current time and expected hit time
                float currentTime = GameHelper.getGlobalTime(); // Current game time
                float closestObjectTime = findClosestObjectTime(cursor.mousePos);
                cursor.mouseDownOffsetMS = (currentTime - closestObjectTime) * 1000; // Offset in milliseconds
                Debug.i("Mouse Down Offset: " + cursor.mouseDownOffsetMS + " ms");
                break;

            case TouchEvent.ACTION_MOVE: // Touch moved
                cursor.mousePos.set(pTouchEvent.getX(), pTouchEvent.getY());
                break;

            case TouchEvent.ACTION_UP: // Touch released
            case TouchEvent.ACTION_CANCEL: // Handle interruptions
